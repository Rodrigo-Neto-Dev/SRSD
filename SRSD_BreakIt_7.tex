...
% ===========================================================================
    ...
    \begin{itemize}
        ...
        \item \textbf{Main result:} The implementation behaved safely in the tested invalid-action cases, but it reported some safe rejections with the wrong return code. It also showed the same room-history duplication issue observed in Group 1. \textbf{Most critically, we identified two security vulnerabilities: weak PBKDF2 parameters that make offline token recovery practical, and a race-condition integrity vulnerability that allows concurrent \texttt{logappend} processes to corrupt the log state, leaving it in an inconsistent condition that permanently rejects all subsequent legitimate operations.}
    \end{itemize}
...
(
    ...
    \subsection{Weak Key Derivation and Offline Brute-Force Vulnerability}

    \subsubsection{High-Level Description}
    During manual code review of the cryptographic implementation, we identified a critical weakness in the password-based key derivation mechanism used by Group 21. Although the implementation uses PBKDF2-HMAC-SHA256, which is normally considered a secure password-hardening function, the iteration count was incorrectly configured to only \textbf{1 iteration}. This completely defeats the intended computational cost of PBKDF2 and makes brute-force and dictionary attacks practical against encrypted logs.

    We successfully reproduced this vulnerability by performing an offline dictionary attack against a protected log file using a standard password wordlist included by default in Kali Linux distributions. The correct token was recovered in a short amount of time without requiring any interaction with the legitimate user or modification of the target log.

    \subsubsection{Root Cause}

    The vulnerability originates in the key-derivation routine:

    \begin{lstlisting}[language=C]
static void gen_key(const char *token,
                    const unsigned char *salt,
                    unsigned char *key) {

    static const unsigned char default_salt[16] =
        "FIXED_SALT_1234";
    ...
    The implementation configures PBKDF2 with only a single iteration:

    \begin{lstlisting}[language=C]
1, EVP_sha256(),
    \end{lstlisting}

    PBKDF2 is specifically designed to slow down brute-force attacks by requiring a large number of repeated cryptographic computations. Modern secure deployments typically use tens or hundreds of thousands of iterations. Using only one iteration makes the derived key almost as cheap to compute as a direct hash operation, enabling extremely fast offline password guessing.

    Additionally, the fallback use of a fixed default salt:
...
static const unsigned char default_salt[16] =
    ...
    further weakens resistance against precomputed attacks and password reuse scenarios.

    \subsubsection{Exploit Preconditions}

    \begin{itemize}
        \item The attacker can obtain a copy of the encrypted log file.
        \item The attacker does not know the authentication token.
        \item The attacker can perform offline password guessing attempts locally.
    \end{itemize}

    \subsubsection{Reproduction Steps}

    The vulnerability was reproduced using a custom Python script named \texttt{crack.py}, which will be submitted together with the project deliverables.

    We used a standard password wordlist bundled with Kali Linux systems to perform the dictionary attack.

    Example execution flow:

    \begin{lstlisting}[language=bash]
# 1. Create or obtain a target log
./logappend -T 1 -K password123 -E Alice -A weak.log

# 2. Execute the offline cracking script
python3 crack.py weak.log \
    /usr/share/wordlists/rockyou.txt

# 3. Script successfully recovers the token
# Output:
# [+] Password found: password123
    ...
    The password was successfully recovered using a standard dictionary attack and a commonly available Kali Linux wordlist. Because PBKDF2 was configured with only one iteration, password guesses could be evaluated extremely quickly, making brute-force attacks practical even on consumer hardware.

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.92\linewidth]{weakEncription_g21.png}
        \caption{Successful offline dictionary attack against Group 21 logs using the provided \texttt{crack.py} script and a standard Kali Linux wordlist.}
        \label{fig:weak-encryption-g21}
    \end{figure}

    \subsubsection{Expected Behavior on a Correct Implementation}

    A secure implementation should use:
    \begin{itemize}
        \item A high PBKDF2 iteration count (typically at least 100000 iterations or more).
        \item A unique cryptographically random salt for every log.
        \item Password-hardening parameters designed to significantly slow offline guessing attacks.
    \end{itemize}

    Under a correct implementation, recovering the token using a standard dictionary attack should be computationally expensive and significantly slower.

    \subsubsection{Security Impact}

    This vulnerability constitutes a serious confidentiality weakness:
    \begin{itemize}
        \item \textbf{Offline brute-force feasibility:} An attacker can recover weak or commonly used tokens efficiently without interacting with the system.
        \item \textbf{Loss of confidentiality:} Once the token is recovered, the attacker can decrypt and inspect protected log contents.
        \item \textbf{Potential integrity compromise:} Knowledge of the token also enables unauthorized appends and manipulation attempts against the log.
        \item \textbf{Scalability of attack:} Because the derivation cost is extremely low, large password dictionaries can be tested rapidly.
    \end{itemize}

    \subsubsection{Attack Artifacts}

    The following files will be submitted together with the report:
    \begin{itemize}
        \item \texttt{attacks/group21/crack.py}
        \item Example vulnerable log files
        \item Reproduction instructions and testcase commands
    \end{itemize}
    ...
    However, \textbf{the race-condition vulnerability G21-A1 is a severe security-relevant finding.} The absence of file locking or atomic read-modify-write primitives allows concurrent \texttt{logappend} processes to corrupt the log state, producing an impossible room-occupancy condition and permanently denying service to all subsequent legitimate operations. This constitutes both an integrity violation (acceptance of an invalid state transition) and a denial-of-service attack against the audit log.

    We additionally identified a serious cryptographic weakness in the password-based key derivation configuration. Although the implementation uses PBKDF2-HMAC-SHA256, it incorrectly configures the derivation process with only one iteration, making offline dictionary and brute-force attacks practical. Using a standard Kali Linux wordlist and our provided \texttt{crack.py} script, we successfully recovered authentication tokens from protected logs. This substantially weakens the confidentiality guarantees of the system.
...
% ===========================================================================
    ...
    \begin{itemize}
        \item \texttt{attacks/group1/exploit.py}: Python script that parses Group 1 encrypted logs, detects AES-GCM IV reuse, and infers per-record structural information (ciphertext length, plaintext length, event-type classification).
        \item \texttt{attacks/group1/}: Additional scripts, modified logs, and README/testcase commands for Group 1.
        \item \texttt{attacks/group21/}: Scripts, modified logs, and README/testcase commands for Group 21.
    \end{itemize}
...
% ===========================================================================
    ...
    Group 21 suffers from two major vulnerabilities. First, the implementation contains a race-condition integrity vulnerability (G21-A1) in which concurrent \texttt{logappend} processes can corrupt the log state through a Time-of-Check to Time-of-Use (TOCTOU) window, permanently denying service to all subsequent legitimate operations. Second, the implementation configures PBKDF2-HMAC-SHA256 with only a single iteration (G21-A2), making offline dictionary and brute-force attacks practical. Using a standard Kali Linux password wordlist and our provided \texttt{crack.py} script, we successfully recovered valid authentication tokens from encrypted logs.
